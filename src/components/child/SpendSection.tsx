...
import { DEFAULT_SPEND_ITEMS } from '@/types';
...
export function SpendSection({ child }: SpendSectionProps) {
  const [quantities, setQuantities] = useState<Record<string, number>>({});
  const [showCart, setShowCart] = useState(false);
  const [isLoading, setIsLoading] = useState(false);

  const spendItems = DEFAULT_SPEND_ITEMS.map((item, idx) => ({
    ...item,
    id: `spend-${idx}`,
  }));
  ...
  const cartItems: CartItem[] = spendItems
