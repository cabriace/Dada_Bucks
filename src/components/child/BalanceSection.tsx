...
export function BalanceSection({ child }: BalanceSectionProps) {
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              <p className="font-black text-[#1A1A2E]">
                {child.pending_earnings} Dada Bucks Coming!
              </p>
              <p className="text-sm text-[#6B6B8C] font-bold">
                Will be added at {RESET_HOUR}:00 PM tonight
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="dada-card dada-card-mint p-6 text-center">
          ...
          <p className="dada-label mb-1">Total Earned</p>
          <p className="text-4xl font-black text-[#15803d]">{child.total_earned}</p>
        </div>

        {/* Strikes */}
        <div className={`dada-card p-6 text-center ${child.strikes > 0 ? 'bg-[#FFB8D0]' : 'bg-white'}`}>
          <div className={`w-14 h-14 rounded-xl flex items-center justify-center mx-auto mb-3 border-[3px] border-[#1A1A2E] shadow-[0_4px_0_#1A1A2E] ${child.strikes > 0 ? 'bg-[#FF6A3D]' : 'bg-white'}`}>
            <AlertTriangle className={`w-7 h-7 ${child.strikes > 0 ? 'text-white' : 'text-[#1A1A2E]'}`} />
          </div>
          <p className="dada-label mb-1">Strikes</p>
          <div className="flex items-center justify-center gap-2">
            <p className={`text-4xl font-black ${child.strikes >= MAX_STRIKES ? 'text-[#FF6A3D]' : 'text-[#1A1A2E]'}`}>
              {child.strikes}
            </p>
      ...
      {child.strikes > 0 && child.strikes < MAX_STRIKES && (
        <div className="dada-card bg-[#FFD200] p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-black text-[#1A1A2E]">
                Careful! You have {child.strikes} strike{child.strikes !== 1 ? 's' : ''}
              </p>
              <p className="text-sm text-[#1A1A2E] font-bold">
                {MAX_STRIKES - child.strikes} more and you lose pending earnings!
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="dada-card bg-white p-4 text-center">
          ...
          <p className="text-2xl font-black text-[#15803d]">{child.total_earned}</p>
          <p className="text-xs text-[#6B6B8C] font-bold uppercase">Total Earned</p>
        </div>
        <div className="dada-card bg-white p-4 text-center">
          <p className="text-2xl mb-1">🎁</p>
          <p className="text-2xl font-black text-[#dc2626]">{child.total_spent}</p>
          <p className="text-xs text-[#6B6B8C] font-bold uppercase">Total Spent</p>
        </div>
        <div className="dada-card bg-white p-4 text-center">
          <PiggyBank className="w-6 h-6 text-[#0369a1] mx-auto mb-2" />
          <p className="text-2xl font-black text-[#0369a1]">{child.savings_balance}</p>
